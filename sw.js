const CACHE='petites-betises-v21';
const SCRIPTS=['photo-fix.js','face-v8.js','romane-pack.js','nora-pack.js','birthday.js'];
const ASSETS=['./','./index.html','./manifest.webmanifest','./nora-photo-originale.webp',...SCRIPTS.map(f=>'./'+f+'?v=21'),...['zero','half','one','two','three','good','birthday'].map(kind=>'./Asset/Nora/nora-'+kind+'-hq.webp')];
...
async function injectFix(response){
  ...
  for(const f of SCRIPTS)if(!body.includes(f))body=body.replace('</body>',`<script src="./${f}?v=21"></script></body>`);
