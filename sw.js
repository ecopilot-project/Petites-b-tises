const CACHE='petites-betises-v24';
const SCRIPTS=['photo-fix.js','face-v8.js','romane-pack.js','nora-pack.js','baptiste-pack.js','sloan-pack.js','birthday.js'];
const ASSETS=['./','./index.html','./manifest.webmanifest','./nora-photo-originale.webp',...SCRIPTS.map(f=>'./'+f+'?v=24'),...['zero','half','one','two','three','good','birthday'].flatMap(kind=>['./Asset/Nora/nora-'+kind+'-hq.webp','./Asset/Baptiste/baptiste-'+kind+'-hq.webp','./Asset/Sloan/sloan-'+kind+'-hq.webp'])];
...
async function injectFix(response){
  ...
  for(const f of SCRIPTS)if(!body.includes(f))body=body.replace('</body>',`<script src="./${f}?v=24"></script></body>`);
