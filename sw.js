const CACHE='petites-betises-v25';
...
const ASSETS=['./','./index.html','./manifest.webmanifest','./nora-photo-originale.webp',...SCRIPTS.map(f=>'./'+f+'?v=25'),...['zero','half','one','two','three','good','birthday'].flatMap(kind=>['./Asset/Nora/nora-'+kind+'-hq.webp','./Asset/Baptiste/baptiste-'+kind+'-hq.webp','./Asset/Sloan/sloan-'+kind+'-hq.webp','./Asset/Raphael/raphael-'+kind+'-hq.webp'])];
...
async function injectFix(response){
  ...
  for(const f of SCRIPTS)if(!body.includes(f))body=body.replace('</body>',`<script src="./${f}?v=25"></script></body>`);
