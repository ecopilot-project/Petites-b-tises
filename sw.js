const CACHE='petites-betises-v22';
const SCRIPTS=['photo-fix.js','face-v8.js','romane-pack.js','nora-pack.js','baptiste-pack.js','birthday.js'];
const ASSETS=['./','./index.html','./manifest.webmanifest','./nora-photo-originale.webp',...SCRIPTS.map(f=>'./'+f+'?v=22'),...['zero','half','one','two','three','good','birthday'].flatMap(kind=>['./Asset/Nora/nora-'+kind+'-hq.webp','./Asset/Baptiste/baptiste-'+kind+'-hq.webp'])];
...
async function injectFix(response){
  ...
  for(const f of SCRIPTS)if(!body.includes(f))body=body.replace('</body>',`<script src="./${f}?v=22"></script></body>`);
